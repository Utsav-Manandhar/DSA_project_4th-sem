...
async function executeAlgorithm()
{
    if(algorithmExecuted || !edgesCreated) return;
        ...
        var scanningNode = 0  // update as we go, initially source node
        nodes[scanningNode].shortestPath = 0;
        displayNodes[scanningNode].style.fill ='url(#orangeRadialGradient)';
        await sleep(1000);
        
        var currentEdges=[];
        var currentAvailablePaths = [];
        ...
        for(let j =0;j < numberOfNodes-1;j++)
        {
            ...
            for(let i =0;i<numberOfEdges;i++)
            {
                if((edges[i].v1 == scanningNode || edges[i].v2 == scanningNode) && edges[i].status==0)
                {
                    ...
                    await sleep(200);
                    var newEdgeObject = {e: edges[i],index :i};
                    currentEdges.push(newEdgeObject);
                }
            }
            for(let i =0;i<currentEdges.length;i++)
            {
                if(currentEdges[i].e.v1 == scanningNode)
                {
                    var newNodeToAdd =currentEdges[i].e.v2;
                }
                else
                {
                    var newNodeToAdd = currentEdges[i].e.v1;
                }
                var newPathLength = Math.min(nodes[newNodeToAdd].shortestPath,nodes[scanningNode].shortestPath + (+currentEdges[i].e.weight));
                var newObject = {n:newNodeToAdd, l: newPathLength,edge:currentEdges[i]};
                if(currentEdges[i].e.status != 2)
                { 
                    currentEdges[i].e.status = 2;
                    currentAvailablePaths.push(newObject);
                }
            }
           
            currentAvailablePaths =  currentAvailablePaths
            .filter((item) => {return nodes[item.n].status == 0;})
            .sort((a,b)=>{return a.l - b.l;});
            var newNodeNumber = currentAvailablePaths[0].n;
            nodes[newNodeNumber].shortestPath = currentAvailablePaths[0].l;
            scanningNode = newNodeNumber;
            nodes[newNodeNumber].status = 1; 
            displayNodes[newNodeNumber].style.fill ='url(#orangeRadialGradient)';
            
            edgeToBeRemoved = currentAvailablePaths[0].edge;
            edgeToBeRemoved.e.status = 3;
            displayEdges[edgeToBeRemoved.index].style.stroke = "url(#yellowLinearGradient)";
            await sleep(1000);

            //console.log(JSON.stringify(currentEdges));
            currentEdges = currentEdges
            .filter((item) => {return item != edgeToBeRemoved;})
            //console.log(JSON.stringify(currentEdges));
            currentAvailablePaths.shift(); 
        }
        for(let i =0;i<numberOfNodes;i++)
        {
            displayNodes[i].style.fill ='url(#greenRadialGradient)';
            await sleep(200);
            console.log(`shortest path to ${i} from source:`);
            console.log(JSON.stringify(nodes[i].shortestPath));
        }
